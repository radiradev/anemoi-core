...
class GraphAutoencoder(pl.LightningModule):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize graph neural network autoencoder.
        ...
        self.grid_dim = -2
        self.rollout = 1  # no actual rollout for the autoencoders, but some callbacks need this
        ...
        assert self.multi_step == 1, "Multi_step must be 1 for the autoencoder model!"
    ...
    def autoencoder_step(
        ...
        Yields
        ------
        tuple[Union[torch.Tensor, None], dict, list]
        ...
        msg = (
            ...
            f", {batch.shape[1]} !>= {self.multi_step}"
        )
    ...
    def _step(
        ...
        loss, metrics, y_preds = self.autoencoder_step(
            batch,
            training_mode=True,
    ...
    def calculate_val_metrics(
        ...
        for metric_name, metric in self.metrics.items():
            if not isinstance(metric, BaseLoss):
                ...
                metrics[f"{metric_name}_metric/1"] = metric(y_pred_postprocessed, y_postprocessed)
                continue

            for mkey, indices in self.val_metric_ranges.items():
                metric_step_name = f"{metric_name}_metric/{mkey}/1"
