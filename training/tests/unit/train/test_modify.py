...
class TestModelModifier:
    ...
    def test_concrete_implementation(self):
        ...
        result = modifier.apply(model, config)
...
class TestFreezingModelModifier:
    ...
    def test_freeze_nonexistent_module(self):
...
class TestTransferLearningModelModifier:
    ...
    def test_init_with_parameters(self):
        ...
        modifier = TransferLearningModelModifier(
            checkpoint_path, strict=True, skip_mismatched=False
        )
        assert modifier.checkpoint_path == checkpoint_path
    ...
    def test_apply(self, mock_load_model_from_checkpoint):
    ...
    @patch('anemoi.training.train.modify.load_model_from_checkpoint')
    def test_apply_with_different_parameters(self, mock_load_model_from_checkpoint):
...
class TestModelModifierApplier:
    ...
    def test_process_no_modifiers(self):
    ...
    def test_process_no_model_modifier_config(self):
    ...
    def test_process_empty_modifiers(self):
    ...
    def test_process_single_modifier(self, mock_instantiate):
        ...
        config = DictConfig({
            "training": {
                "model_modifier": {
                    "modifiers": [
                        {
                            "_target_": "test.modifier.TestModifier"
                        }
                    ]
                }
            }
    ...
    @patch('anemoi.training.train.modify.instantiate')
    def test_process_multiple_modifiers(self, mock_instantiate):
        ...
        config = DictConfig({
            "training": {
                "model_modifier": {
                    "modifiers": [
                        {"_target_": "test.modifier.Modifier1"},
                        {"_target_": "test.modifier.Modifier2"}
                    ]
                }
            }
        })
    ...
    def test_process_modifier_order(self):
...
class TestModelModifierIntegration:
    ...
    @patch('anemoi.training.train.modify.load_model_from_checkpoint')
    def test_transfer_learning_after_freezing(self, mock_load_model_from_checkpoint):
        ...
        model = transfer_modifier.apply(model, config)
        
        # Then freeze encoder  
        freeze_modifier = FreezingModelModifier(["encoder"])
        model = freeze_modifier.apply(model, config)
    ...
    @patch('anemoi.training.train.modify.load_model_from_checkpoint')
    def test_full_pipeline_with_applier(self, mock_load_model_from_checkpoint):
