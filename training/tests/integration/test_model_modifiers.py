...
class TestModelModifiersIntegration:
        ...
        base_config = {
            ...
            "hardware": {
                ...
                "paths": {
                    ...
                    "plots": "/tmp/test_plots",
                    "logs": {
                        "tensorboard": "/tmp/test_logs"
                    }
                },
                "files": {
                    "graph": None,
            ...
            "model": {
                "output_mask": {
                    "_target_": "test.MockOutputMask"
                },
                "keep_batch_sharded": False
            },
            "datamodule": {
                "_target_": "test.MockDataModule"
            },
            "dataloader": {
                "read_group_size": 1,
                "grid_indices": {
                    "_target_": "test.MockGridIndices"
                },
                "limit_batches": {
                    "training": 1.0,
                    "validation": 1.0
    ...
    @patch('anemoi.training.train.train.AnemoiTrainer.truncation_data', new_callable=lambda: {})
    @patch('anemoi.training.train.train.AnemoiTrainer.metadata', new_callable=lambda: {})
    @patch('anemoi.training.train.train.AnemoiTrainer.supporting_arrays', new_callable=lambda: {})
    @patch('anemoi.training.train.train.get_class')
    def test_model_modifier_integration(self, mock_get_class, mock_supporting_arrays, 
                                       mock_metadata, mock_truncation_data, 
                                       mock_graph_data, mock_datamodule):
    ...
    @patch('anemoi.training.train.train.AnemoiTrainer.supporting_arrays', new_callable=lambda: {})
    @patch('anemoi.training.train.train.get_class')
    def test_no_model_modifier_config(self, mock_get_class, mock_supporting_arrays, 
                                     mock_metadata, mock_truncation_data, 
                                     mock_graph_data, mock_datamodule):
    ...
    @patch('anemoi.training.train.train.AnemoiTrainer.supporting_arrays', new_callable=lambda: {})
    @patch('anemoi.training.train.train.get_class')
    def test_multiple_modifiers_integration(self, mock_get_class, mock_supporting_arrays, 
                                           mock_metadata, mock_truncation_data, 
                                           mock_graph_data, mock_datamodule):
    ...
    def test_model_modifier_config_validation(self):
        ...
        assert config.training.model_modifier.modifiers[0]["_target_"] == "anemoi.training.train.modify.WeightsInitModelModifier"
    
    @pytest.mark.longtests
    @patch('anemoi.training.train.train.AnemoiTrainer.train')
    def test_skip_checkpoint_loading_integration(self, mock_train_method):
...
class TestModelModifierConfigTemplates:
    ...
    def test_freezing_config_structure(self):
    ...
    def test_enhanced_fine_tuning_config_structure(self):
...
class TestModelModifierErrorHandling:
    ...
    @patch('anemoi.training.train.train.AnemoiTrainer.supporting_arrays', new_callable=lambda: {})
    @patch('anemoi.training.train.train.get_class')
    def test_invalid_modifier_target(self, mock_get_class, mock_supporting_arrays, 
                                    mock_metadata, mock_truncation_data, 
                                    mock_graph_data, mock_datamodule):
        ...
        config = self.create_minimal_config(**{
            "training.model_modifier": {
                "modifiers": [
                    {
                        "_target_": "nonexistent.module.NonexistentModifier"
                    }
                ]
            }
        })
    ...
    @patch('anemoi.training.train.train.AnemoiTrainer.supporting_arrays', new_callable=lambda: {})
    @patch('anemoi.training.train.train.get_class')
    def test_modifier_exception_handling(self, mock_get_class, mock_supporting_arrays, 
                                        mock_metadata, mock_truncation_data, 
                                        mock_graph_data, mock_datamodule):
        ...
        class FailingModifier(ModelModifier):
            def apply(self, model, config):
                raise ValueError("Test exception from modifier")
        ...
        with patch('anemoi.training.train.modify.instantiate') as mock_instantiate:
            ...
            test_config = DictConfig({
                "training": {
                    "model_modifier": {
                        "modifiers": [{"_target_": "test.FailingModifier"}]
                    }
                }
            })
