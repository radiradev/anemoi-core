...
class AnemoiModelTwoStepDec(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self._graph_name_intermediate = model_config.graph.intermediate
        ...
        # Decoder hidden -> int
        self.decoder1 = instantiate(
            model_config.model.decoder,
            _recursive_=False,  # Avoids instantiation of layer_kernels here
            in_channels_src=self.num_channels,
            in_channels_dst=self.node_attributes.attr_ndims[self._graph_name_hidden],
            hidden_dim=self.num_channels,
            out_channels_dst=self.num_channels,
            sub_graph=self._graph_data[(self._graph_name_hidden, "to", self._graph_name_intermediate)],
            src_grid_size=self.node_attributes.num_nodes[self._graph_name_hidden],
            dst_grid_size=self.node_attributes.num_nodes[self._graph_name_intermediate],
        )
        ...
        self.decoder = instantiate(
            ...
            sub_graph=self._graph_data[(self._graph_name_intermediate, "to", self._graph_name_data)],
            src_grid_size=self.node_attributes.num_nodes[self._graph_name_intermediate],
    ...
    def forward(
        ...
        **kwargs,
        ...
        x_intermediate_latent = self.node_attributes(self._graph_name_intermediate, batch_size=batch_size)
        shard_shapes_hidden = get_shard_shapes(x_hidden_latent, 0, model_comm_group)
        shard_shapes_intermediate = get_shard_shapes(x_intermediate_latent, 0, model_comm_group)
        ...
        x_out = self._run_mapper(
            self.decoder1,
            (x_latent_proc, x_intermediate_latent),
            batch_size=batch_size,
            shard_shapes=(shard_shapes_hidden, shard_shapes_intermediate),
            model_comm_group=model_comm_group,
        ...
        x_out = self._run_mapper(
            ...
            (x_out, x_data_latent),
            batch_size=batch_size,
            shard_shapes=(shard_shapes_intermediate, shard_shapes_data),
            model_comm_group=model_comm_group,
            x_src_is_sharded=True,  # x_latent always comes sharded
            x_dst_is_sharded=in_out_sharded,  # x_data_latent comes sharded iff in_out_sharded
            keep_x_dst_sharded=in_out_sharded,  # keep x_out sharded iff in_out_sharded
